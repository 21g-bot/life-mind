...
聊天窗口的“清空记录”会清除本机聊天原文及后续模型上下文，包括心智事件中的原文副本。
它不会自动删除已经单独准入的长期记忆；需要彻底忘记某个稳定事实时，再到“管理本地记忆”
中删除对应项目。两种删除入口刻意分开，避免误操作。

主动清除聊天或删除记忆后，程序会压缩活动数据库，并把普通备份轮换成一份新的脱敏快照。
`recovery\` 是数据库曾经损坏时留下的隔离副本，为避免误删唯一可恢复数据，程序不会自动
移除它；确认当前数据库和新备份健康后，可由用户自行删除不再需要的历史隔离目录。
